...
<div class="container py-5">
    ...
    <div class="mt-2 p-3 text-theme2 bg-theme3 rounded">
        <form action="/admin/categories/{{$category->name}}" method="POST" enctype="multipart/form-data">
            @method('put')
            @csrf
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" id="name" placeholder="Cth. Minuman" required autofocus value="{{old('name',$category->name)}}">
                @error('name')
                    <div class="invalid-feedback">
                        {{$message}}
                    </div>
